...
**Focused verification:** start the UI with `npm --prefix ui run dev`, then run
`npm run visual:ray` after locally generating explicitly reviewed ray goldens with
`npm run visual:ray:update`; run the unchanged splat suite with `npm run visual` (alias
`npm run visual:splat`); finish with `npm test -- --runInBand` and
...
| RT-013 | Complete | `2d62736`, `718df72`, `505fb94`, `4f69943`; shared sampling contract, deterministic CPU oracle, rgba16float readback, RMSE/SSIM gate, and Chrome WebGPU validation |
...
Hybrid glTF shadow checkpoint (2026-08-18): Chrome `151.0.7922.138` on macOS loaded the
deterministic `ShadowReceiversAndCasters` fixture through the production file picker and
supported local `.gltf` + `.bin` path, selected `Hybrid ray-traced shadows` through the
modular Settings control, enabled stats, and paused rendering. The `540 x 708` viewport
contained three raster primitives and three matched RayScene instances: a floor receiver
plus red and blue box casters under a directional light. Both boxes produced stable hard
cast/contact shadows. Browser capture reported no error banner, page/console errors, or
WebGPU validation error; sampled GPU timings were approximately `0.13 ms` G-buffer,
`0.66 ms` shadow, and `0.20 ms` composite. Attach
`/tmp/drishyam-gltf-hybrid-ui.png` and `/tmp/drishyam-gltf-hybrid-controls.png` to the PR
rather than committing them. The production UI build and all `40` Jest suites (`295`
...
Automated ray release checkpoint (2026-08-21): `npm run visual:ray` passed in Chrome
`151.0.7922.172` on macOS with the Intel `gen-9` WebGPU adapter. Both parity cases used a
`32 x 32` linear `rgba16float` readback, fixed seed `0x12345678`, and `32` SPP. The
one-bounce case measured RMSE `0.000034` and SSIM `1.000000`; the four-bounce case measured
RMSE `0.000242` and SSIM `0.999994`. The enforced cross-device tolerance is RMSE `<= 0.005`
and SSIM `>= 0.995`; stack-overflow and non-finite diagnostic counts must both equal zero.
The camera-reset case synchronously changed SPP from `32` to `0` and resumed from `16`.

The same run captured the real worker-backed CPU Cornell output, direct and multi-bounce GPU
Cornell output, the moved-camera reset frame, and a `320 x 320` hybrid glTF hard-shadow
frame. The hybrid fixture is generated by `visual/shadow-fixture.mjs` and contains a plane
receiver plus one box caster (`14` triangles and `2` RayScene instances), so it carries no
external asset license. The default scene animation is replaced with a no-op script during
that case to keep the camera, model transform, and shadow stable. The report recorded zero
browser/page errors and hybrid timings near `0.20 ms` G-buffer, `4.06 ms` shadow, and
`0.13 ms` composite on this adapter. Local reviewed goldens live under the gitignored
`visual/golden/ray/`; current PNGs and `report.json` live under `visual/output/ray/`. Attach
those small PNGs to the PR rather than committing them.

Ray harness configuration may be overridden under a `ray` object in the gitignored
`visual/local.config.json`. Do not loosen parity or pixel thresholds merely to update a
golden: first inspect the linear metrics, diagnostics, diff PNG, adapter metadata, and the
source change. `npm run visual:ray:update` is an explicit review action; ordinary validation
uses `npm run visual:ray`. The original `visual/cases.mjs` splat matrix and
`visual/run-visual.mjs` behavior remain unchanged. The final gate passed all `41` Jest suites
