...
**Relevant files**
- scripts/engine/scene.js - migrate from single drawable assumptions to multi-object state and per-object materials.
- scripts/engine/app-facade.js - define stable APIs for scene manipulation, serialization, and backend switching.
- scripts/engine/scene-ops.js - extend add/duplicate/delete/reset operations and import/export hooks.
- scripts/engine/gltf-parser.js - ensure imported glTF path maps to parity material/object contracts.
- scripts/engine/webgpu-facade.js - parity wiring and feature gates for WebGPU workflows.
- scripts/engine/webgpu-scene.js - instancing and splat rendering integration.
- scripts/engine/webgpu-helpers.js - buffer layouts, resource binding updates, and GPU guardrails.
- assets/shaders/default.wgsl - parity shading path and extensibility points for material controls.
- ui/src/App.jsx - feature-sliced UI orchestration for inspector/shader/persistence/backend controls.
- __tests__/scene.test.js - expand for multi-object behavior and serialization interactions.
- __tests__/webgl-helpers.test.js - baseline references for rendering helper correctness.
- __tests__/geometry.test.js - fixture extensions for parity geometry workflows.
- docs/webgpu-migration-plan.md - update with parity completion criteria and splat viewer milestones.
